...
# -*- coding: utf-8 -*-
"""
Shaxsiy AI Yordamchi v6.0 — Premium Edition
- Voice O'CHIRILDI
- To'liq Inline Keyboard
- Zichlangan Xotira
- Localhost OmniRouter uchun optimallashtirilgan
"""
...
# !!! MUHIM: AI_BASE_URL ni localhostga o'zgartiring !!!
...
async def ai_chat(messages: list, temperature: float = 0.7, max_tokens: int = 4000, model_idx: int = 0) -> str:
    if model_idx >= len(MODELS):
        raise RuntimeError("AI model topilmadi.")
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(f"{AI_BASE_URL}/chat/completions", headers=headers, json=payload, timeout=aiohttp.ClientTimeout(total=60)) as resp:
                    ...
                    if resp.status in [401, 402, 403]:
                        return "❌ OmniRouter'da kredit tugagan, kalit bloklangan yoki ruxsat yo'q."
                    if resp.status != 200:
    ...
    raise RuntimeError("AI so'rovi bajarilmadi.")
...
async def cmd_memory_compress(message: Message):
    ...
    await message.answer("🧠 Xotira zichlanmoqda, biroz kuting...")
    ...
    try:
        compressed_memory = await ai_chat([{"role": "system", "content": "Siz xotira analizchisisiz."}, {"role": "user", "content": prompt}], temperature=0.3, max_tokens=4000)
...
async def menu_export(callback: CallbackQuery):
    ...
    try:
        ...
        if filename:
            await callback.message.answer_document(FSInputFile(filename), caption="🔐 Shifrlangan eksport.")
            os.remove(filename)
        else:
...
async def menu_reminder(callback: CallbackQuery):
    await callback.answer()
    await callback.message.answer("⏰ Foydalanish: /eslatma 2026-08-22 15:00 Uchrashuvga borish")
...
async def _notify_if_slow(chat_id: int):
    ...
    try:
        await bot.send_message(chat_id, "⏳ AI javob tayyorlanmoqda...\nBu 10-30 soniya olishi mumkin (model band bo'lsa).")
    except:
        pass
