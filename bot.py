...
# -*- coding: utf-8 -*-
"""
Shaxsiy AI Yordamchi v5.0 — OmniRouter Combo Edition
- OmniRouter Combo (5 ta Groq kaliti) bilan ishlash uchun optimallashtirilgan
- Fallback butunlay o'chirilgan (faqat Combo modeli ishlaydi)
- Xatolarni aniq va tushunarli qilib qaytaradi (429, 401, 402 va h.k.)
- IP bloklanishining oldini olish uchun OmniRouter Combo'ga tayanadi
"""
...
# !!! MUHIM !!!: Bu yerda faqat bitta model nomi bo'ladi. 
# OmniRouter Combo'ning o'zi Round-robin bilan barcha kalitlarni ishlatadi.
# Siz .env faylida AI_MODEL ga "groq/openai/gpt-oss-120b" deb yozgan edingiz.
# Bu model OmniRouter'ning Combo ichidagi barcha 5 ta kalitga taqsimlanadi.
MODELS = [os.getenv("AI_MODEL", "groq/openai/gpt-oss-120b")]
...
def init_db():
    ...
    logger.info("✅ Baza v5.0 initializatsiya qilindi (indexlar bilan)")
...
# ============== AI CLIENT (OmniRouter Combo Edition) ==============
async def ai_chat(messages: list, temperature: float = 0.7, max_tokens: int = 4000, model_idx: int = 0) -> str:
    """OmniRouter Combo orqali ishlaydigan, xatolarni aniq ko'rsatadigan AI funksiyasi"""
    ...
    # 3 marta urinish (429 yoki vaqtinchalik xatolar uchun)
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    headers=headers,
                    json=payload,
                    timeout=aiohttp.ClientTimeout(total=60)  # OmniRouter Combo uchun 60s qulay
                ) as resp:
                    text = await resp.text()

                    # 429 - Rate Limit yoki Combo band
                    if resp.status == 429:
                        wait = min(2 ** attempt * 2, 10)
                        logger.warning(f"[{model}] 429 - Limiting, {wait}s kutamiz... (urinish {attempt+1})")
                        await asyncio.sleep(wait)
                        continue

                    # 401, 402, 403 - Kalit, kredit yoki ruxsat xatosi
                    ...
                    # Boshqa server xatolari (500 va h.k.)
                    if resp.status != 200:
            ...
            logger.warning(f"[{model}] Timeout (60s) — urinish {attempt+1}")
            if attempt < 2:
    ...
    raise RuntimeError("AI so'rovi bajarilmadi. OmniRouter Combo holatini tekshiring yoki keyinroq urinib ko'ring.")
...
async def main():
    ...
    logger.info("✅ Bot ishga tushdi (v5.0 — OmniRouter Combo Edition)")
